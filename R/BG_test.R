...
#'
#' This function tests the serial correlation of residuals using the Breusch-Godfrey test. The user can choose between three versions of the test: likelihood ratio (\code{LR}), Lagrange multiplier (\code{LM}) and Rao.
#'
#' @param object An object of class \code{VECM}.
#' @param lag The lag order of auxiliary regression model.
#' @param type The test version. It can be selected between \code{LR} (the default), \code{LM} and \code{Rao}.
#'
...
BG_test <- function(object, lag, type = c("LR", "LM", "Rao")) {
  ...
  if (type == "Rao") {
    ...
    q <- 0.5 * K * m - 1
    z <- delta - m - 0.5 * (K - m + 1)
    stat <- ((det(sigma_H0) / det(sigma_H1))^(1 / s) - 1) * (z * s - q) / (K * m)
    df2 <- z * s - q
    stat_pval <- pf(q = stat, df1 = df, df2 = df2, lower.tail = FALSE)
  }

  if (type == "Rao") {
    output <- list("stat" = stat, "stat_pval" = stat_pval, "df" = c(df, df2), "type" = type, "lag" = lag)
  } else {
